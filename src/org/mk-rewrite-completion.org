...
Consider the set of equations--representing some standard group axioms
(e.g. inverses, identity elements, associativity)--given by the following:
\begin{equation}
  \begin{aligned}
    1 \cdot x &= x
    \\
...
Our interest is in finding a "complete" set of rewrite rules--when possible--for
such equations expressed as miniKanren
[[citep:ByrdRelationalProgrammingminiKanren2009]] relations.  For instance, the
equations in [[Cref:eq:group-axioms]] might be formulated as relations
like src_scheme{(fresh (x ...) (== `(* 1 ,x) 1) ...)}.

Completion for a set of rewrite rules affects ones ability to determine the validity
of equational statements--among other things.  For example, under
[[Cref:eq:group-axioms]] is \(x \cdot x^{-1} = 1\) also true/valid?.

In general, miniKanren implements a complete search
[[citep:HemannSimpleCompleteSearch2018, Hemannsmallembeddinglogic2016]] that can
account for some of the concerns addressed by a complete set of rewrite rules.
However, standard miniKanren relations that effectively implement the basic
tasks of term rewriting can suffer from performance issues resulting from the
inherently large search spaces induced by the relations.

While some of these issues can be mitigated through alterations to the search process
[[citep:SwordsGuidedSearchminiKanren, ZhangNeuralGuidedConstraint2018,
BoskinSurprisinglyCompetitiveConditional2018]], these approaches introduce more
opportunities to unknowingly violation the relational paradigm offered by
miniKanren, but, more importantly, they require a distinct effort from the developer
to provide context-dependent logic that actually *does* improve the search.
...
:NOTES:
Ultimately, the process we're describing generates a reduced set
[[citep:HarrisonIntroductionlogicautomated2007][Theorem 4.26]] of "directional"
rewrite relations that represent a completed set of rewrite rules.

The idea is that the completed rules, \(l \to r\), when expressed
as src_scheme{(conde ((fresh (l r ...) (== l ...) (== r ...) ...)))}, are
minimal--in some "overlap" sense that reduces the number of effective matches
across src_scheme{conde} branches--and sufficient to represent the original
equations/relations.
:END:

As a further elaboration to some of the points above, consider the rewrite rules
implied by the left-to-right replacements in [[Cref:eq:group-axioms]].  Under these
rewrite rules we obtain the following normal form for \((x^{-1} \cdot x) \cdot y\)
through repeated application of the rewrite rules in order (see
...
"reduces" terms in some sense, and--as a result--will produce nice normal forms.
However, in a more general context, we need a means of automatically deriving
rules with such properties (and more).

Now, consider the normal form for \((x \cdot x^{-1}) \cdot y\); as with our example above, it should
be \(y\).  Unfortunately, our naively derived rewrite rules will not
produce the expected normal form, because the identity \(x \cdot x^{-1}\) is not
present--although it is implied by [[Cref:eq:group-axioms]].

Unfortunately, the addition of new rewrite rules to compensate for this deficiency can easily
remove any reducibility guarantees afforded by our present rules.  For instance, if
we add commutativity \(x \cdot y \to y \cdot x\), even in more restricted
forms, we will produce a non-terminating rewrite system.

:TODO:
Will miniKanren actually arrive at this identity?  Use the graph relations below
to confirm/demonstrate.
At the very least, demonstrate how much work is done by miniKanren to arrive
at/validate the identity
:END:

A complete set of rewrite rules for [[Cref:eq:group-axioms]] is given by the following:
\begin{equation}
  \begin{aligned}
    1 \cdot x &\to x
    \\
...
[[Cref:eq:group-axioms]]--and a generic reduction order--using standard approaches
to computing a Knuth-Bendix completion (e.g. Huet's procedure).  As we can see,
the commutative inverse identity (i.e. \(x^{-1} \cdot x = x \cdot x^{-1}\)) is
present in a form that preserves termination, along with other important
rewrites that we might have easily missed when attempting to manually produce a
