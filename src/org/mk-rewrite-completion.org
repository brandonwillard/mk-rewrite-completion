...
(define (-normalize rules t)
  (match t
    [(cons t-car t-cdr)
     (let ([t-cdr-new (for/list ([a t-cdr])
                        (-normalize rules a))])
       (if (eq? t-cdr-new #f)
           t
           (begin
...
(define (rewrite rules t)
  (apply-on-grounded (curry -rewrite rules) t))
#+END_SRC
...
(define (norm-orient ord rules eq)
  (match eq
    [(cons s t)
...
(let* ([ord (curry lpo-gt '(* i 1))]
       ...
       [res (norm-orient ord rules eq)])
  (check-equal? res `((* ,x (i ,x)) . 1)))
...
# eval: (progn (put 'rewrite-rules 'scheme-indent-function 1) (put 'fresh 'scheme-indent-function 1))
# End:
