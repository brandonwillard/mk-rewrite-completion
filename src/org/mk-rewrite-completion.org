...
In [[mk-algebra-rewrite-example-defs]] we only considered the rewrite rules for
...
As we mentioned earlier, a naively derived set of rewrite rules \(\mathcal{R}\)
...
Starting with a simple set of rewrite rules, the classical
approach to completion [[citep:KnuthSimplewordproblems1983]] produces a
confluent, terminating set of rewrite rules--when possible.  This is done by
recursively generating and normalizing the critical pairs resulting from a
running set of rewrite rules and adding distinct pairs as new rewrite rules.
When all the distinct critical pairs are exhausted, the resulting rule set is
confluent.
...
In [[basic-completion-group-example]], we run the standard completion procedure
...
[[basic-completion-group-rewrite-example]] using only the same initial rewrite
rules--this time expressed as relations--we would find that its relational
counterpart in miniKanren is also unable to reduce the term.
We demonstrate this in Listing [[mk-algebra-rewrite-example-revisited]].
...
:TODO:
Use the output of [[basic-completion-group-example]] to generate new src_scheme{conde}
rewrite rules.
:END:
...
** Rewrite Relations                                              :noexport:

#+NAME: rewrite-rules-relation-macro
#+BEGIN_SRC scheme :eval never :exports none :results none
(define-syntax (rewrite-ruleso stx)
  (syntax-case stx (->)
    [(_ l-var r-var (x ...) (l -> r) ...)
     #'(conde
        [(fresh (x ...)
